...
def train_epoch(model, dataloader, optimizer, scheduler, device):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        # Clear cache periodically and show progress
        if (batch_idx + 1) % 10 == 0:
            torch.cuda.empty_cache()
...
def train_classical_model(train_tfidf, val_tfidf, train_labels, val_labels, tfidf_vectorizer):
    ...
    print(f"Logistic Regression saved to {MODELS_DIR}/lr_model.pkl")
...
def main():
    ...
    lr_model, lr_training_time = train_classical_model(train_tfidf, val_tfidf, train_labels, val_labels, tfidf_vectorizer)
